...
namespace DotNetCoreKoans.Koans
{
    public class AboutArrays : Koan
    {
        ...
        public void PushingAndPopping()
        {
            ...
            Object[] expectedPushedOutput = { "last", 2, 1 };
            Object[] expectedPoppedOutput = { 2, 1 };
            stack.Push("last");
            Assert.Equal(expectedPushedOutput, stack.ToArray());
            var poppedValue = stack.Pop();
            Assert.Equal("last", poppedValue);
            Assert.Equal(expectedPoppedOutput, stack.ToArray());
        }
        ...
        public void Shifting()
        {
            ...
            String[] addFirst = { "Say", "Hello", "World" };
            String[] removeLast = { "Say", "Hello" };
            String[] removeFirst = { "Hello" };
            String[] addAfter = { "Hello", "World" };

            list.AddFirst("Say");
            Assert.Equal(addFirst, list.ToArray());

            list.RemoveLast();
            Assert.Equal(removeLast, list.ToArray());

            list.RemoveFirst();
            Assert.Equal(removeFirst, list.ToArray());

            list.AddAfter(list.Find("Hello"), "World");
            Assert.Equal(addAfter, list.ToArray());
        }
